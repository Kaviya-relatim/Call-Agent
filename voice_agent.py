...
async def entrypoint(ctx: JobContext):
    ...
    if not is_outbound:
        ...
        # For outbound calls, introduce yourself when they answer
        await session.generate_reply(
            instructions="Introduce yourself as the Relatim assistant calling about their service jobs. Ask if this is a good time to talk."
        )
    
    logger.info(f"Agent started and ready for conversation (outbound={is_outbound})")
