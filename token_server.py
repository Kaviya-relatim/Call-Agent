...
# SIP Outbound Trunk ID - get from LiveKit Cloud
# Available trunks: ST_tNA4ThVu9bbv (Vobiz Outbound), ST_7zPR4BWU5Hjb (Vobiz-Outbound-Trunk)
LIVEKIT_SIP_TRUNK_ID = os.getenv("LIVEKIT_SIP_TRUNK_ID", "ST_tNA4ThVu9bbv")
...
async def make_call(request: CallRequest):
    ...
    Dispatches the AI agent to a new room with the phone number in metadata.
    The agent will then place the outbound call.
    """
    try:
        ...
        print(f"Using SIP Trunk ID: {trunk_id}")
        ...
        await livekit_api.room.create_room(api.CreateRoomRequest(
            name=room_name,
            empty_timeout=60,  # Close room after 60s if empty
            max_participants=10
        ))
        print(f"Room created: {room_name}")
        
        # Create SIP participant (place the outbound call)
        sip_request = api.CreateSIPParticipantRequest(
            sip_trunk_id=trunk_id,
            sip_call_to=request.phone,
            room_name=room_name,
            participant_identity=f"phone-{request.phone.replace('+', '')}",
            participant_name="Phone User",
            wait_until_answered=False
        )
        
        result = await livekit_api.sip.create_sip_participant(sip_request)
        print(f"SIP participant created: {result}")
        
        # Dispatch agent to the room with metadata
        import json
        metadata = json.dumps({"phone_number": request.phone, "outbound": True})
        ...
        return {
            ...
            "room_name": room_name,
            "phone": request.phone,
            "sip_call_id": result.sip_call_id if hasattr(result, 'sip_call_id') else None
        }
        
    except api.TwirpError as e:
        raise HTTPException(
            status_code=500,
            detail=f"LiveKit SIP error: {e.message}, SIP status: {e.metadata.get('sip_status_code', 'N/A')} {e.metadata.get('sip_status', '')}"
        )
